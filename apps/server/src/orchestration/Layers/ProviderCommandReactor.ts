...
const make = Effect.gen(function* () {
  ...
  const ensureSessionForThread = Effect.fnUntraced(function* (
    ...
    const threadProvider: ProviderKind =
      currentProvider ??
      (options?.provider ?? null) ?? // explicit provider on first turn
      (options?.model ? inferProviderForModel(options.model, "codex") : null) ?? // infer from model
      "codex"; // ultimate fallback (should rarely hit)
